...
def CountP2(test):
    ...
    for i in  range(0,60):
        sum*=getPro(())
'''
X = np.loadtxt('4texture.txt',delimiter=',', dtype=np.float)
# 其中有97
m = X.shape[1] - 1  # 属性数量
print(m)
n = X.shape[0]  # 样本数目
...
print(Class10)
print(Class11)
X=np.vstack((array0,array1,array2,array3,array4,array5,array6,array7,array8,array9,array10))
print(X)
...
for i in range(0, 20):
    ...
    trainingSet = np.vstack((Data1, Data2, Data3, Data4,Data5,Data6,Data7,Data8,Data9,Data10,Data11))
    ...
    acc.append((add + add1 + add2 + add3+add4+add5 + add6 + add7 + add8+ add9+add10 ) / (test1 + test2 + test3 + test4+test5 + test6 + test7 + test8+test9 + test10+ test11 ))
    print("accuracy:{:.2%}".format((add + add1 + add2 + add3+add4+add5 + add6 + add7 + add8+ add9+add10 ) / (test1 + test2 + test3 + test4+test5 + test6 + test7 + test8+test9 + test10+ test11)))
