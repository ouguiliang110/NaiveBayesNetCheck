...
class NBayesPSO(object):
    ...
    def __init__(self, parameters, trainSet):
        ...
        self.PNKArray = self.CountPNKWeight(trainSet)
        self.PNKArrayInstance=self.computePNKNotWeight(trainSet)
    ...
    def fitness(self, W):
        ...
        for i in range(self.N):
            for k in range(self.K):
                ...
                for d in range(self.D):
                    all*=self.ConditionArray[i,k,d]**W[d]
                Eadd2+=((all * self.p_prior[k + 1]) / PxnArray[i] -self.PNKArrayInstance[i,k])**2
    ...
    def CountPNKWeight(self, trainingSet):
    ...
    def computePNKNotWeight(self, trainingSet):
    ...
    def train(self, vector_data, label_data):
        ...
        pso = PSO(func = self.fitness, dim = self.D, pop = 40, max_iter = 150, lb = [0.00001] * self.D,
                  ub = [0.99999] * self.D, w = 0.8, c1 = 0.5, c2 = 0.5)
        pso.run()
        self.Wi = pso.gbest_x
        print(pso.gbest_x)
...
if __name__ == "__main__":
    ...
    X = np.loadtxt('../数据集/[011]haberman(0-1).txt')
