...
class NBayesPSO(object):
    ...
    def train(self, vector_data, label_data):
        ...
        pso=PSO(func= self.fitness,dim=self.D,pop = 20,max_iter = 20,lb = [0.00001]*self.D,ub=[0.99999]*self.D,w = 0.8,c1=0.5,c2=0.5)
        pso.run()
        self.Wi=pso.gbest_x
        print('list每个迭代的gbest_Y', list(pso.gbest_y_hist))
        matrix=[]
        for i in pso.gbest_y_hist:
            matrix.append(list(i))
        self.tagmatrix=list(np.array(matrix).flatten())
        print(self.tagmatrix)
        #print("pbest_x", pso.pbest_x)
        for i in pso.pbest_x:
            x_norm = np.linalg.norm(i)
...
if __name__ == "__main__":
    ...
    length=20
    #weightmatrix=[1.119, 1.120, 1.104, 1.099, 1.155, 1.078, 1.125, 1.137, 1.123, 1.115, 1.115, 1.115, 1.115, 1.115, 1.115, 1.115, 1.115, 1.115, 1.115, 1.115, 1.115, 1.115]
    Good_test_Accuracy=np.array(weightmatrix)
    ling_mean=np.mean(Good_test_Accuracy,axis = 0)
    print(list(ling_mean))
    '''
    ...
    plt.plot(range(len(weightmatrix)), weightmatrix, '-')
    #plt.fill_between(range(length), ling_std1, ling_std2, alpha = 0.3)
    # plt.ylim((0.5, 1))
    plt.xlabel('Increment Times')
    plt.ylabel('Weight')
    ...
    ling_std1 = np.mean(Good_test_Accuracy, axis = 0) + np.std(Good_test_Accuracy, axis = 0)
    ling_std2 = np.mean(Good_test_Accuracy, axis = 0) - np.std(Good_test_Accuracy, axis = 0)
    print(list(np.std(Good_test_Accuracy, axis = 0)))

    print(list(ling_mean))
    print(list(ling_std1))
    print(list(ling_std2))
