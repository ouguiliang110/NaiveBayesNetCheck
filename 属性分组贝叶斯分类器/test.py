...
# 确认数据集，验证集，测试集区
train_index1 = [45, 68, 140, 99, 21, 2, 5, 32, 18, 74, 39, 20, 79, 125, 94, 34, 115, 109, 136, 9, 129, 138, 4, 62, 55,
                127, 77, 31, 133, 36, 76, 117, 38, 85, 54, 10, 35, 96, 57, 142, 144, 33, 130, 75, 100, 118, 47, 135, 69,
                114, 46, 139, 101, 50, 58, 90, 1, 105, 11, 51, 137, 25, 93, 70, 88, 28, 104, 119, 23, 59, 89, 53, 44]
val_index1 = [13, 102, 73, 30, 7, 91, 66, 12, 82, 112, 86, 111, 97, 27, 52, 67, 123, 143, 81, 41, 126, 145, 6, 64, 98,
              83, 106, 84, 95]
test_index1 = [0, 3, 8, 14, 15, 16, 17, 19, 22, 24, 26, 29, 37, 40, 42, 43, 48, 49, 56, 60, 61, 63, 65, 71, 72, 78, 80,
               87, 92, 103, 107, 108, 110, 113, 116, 120, 121, 122, 124, 128, 131, 132, 134, 141, 146]
...
pop_size = 10 # 种群大小
var_num = m * 2  # 变量个数
pop_x = np.random.dirichlet(np.ones(var_num), size = pop_size) * 10
pop_v = np.zeros((pop_size, var_num))
p_best = np.zeros((pop_size, var_num))  # 每个粒子最优的位置
g_best = np.zeros((1, var_num))  # 全局最优的位置
# 初始化第0代初始全局最优解
temp = -1
for i in range(pop_size):
    for j in range(var_num):
        pop_v[i][j] = random.uniform(0, 1)
    p_best[i] = pop_x[i]  # 存储最优的个体
    fit = fitness(p_best[i])
    if fit > temp:
        g_best = p_best[i]
        temp = fit

def update_operator(pop_size):
    c1 = 2
    c2 = 2
    w = 0.4
    for i in range(pop_size):
        # 更新速度
        pop_v[i] = w * pop_v[i] + c1 * random.uniform(0, 1) * abs(
            p_best[i] - pop_x[i]) + c2 * random.uniform(0, 1) * abs(g_best - pop_x[i])
        # 更新位置
        pop_x[i] = pop_x[i] + pop_v[i]
        # 越界保护

        # 更新p_best和g_best
        if fitness(pop_x[i]) > fitness(p_best[i]):
            p_best[i] = pop_x[i]
        if fitness(pop_x[i]) > fitness(g_best):
            g_best = pop_x[i]
