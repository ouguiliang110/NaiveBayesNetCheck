...
A2=np.random.uniform(0,1,size=sample)
dataset.append(A1)
dataset1.append(A2)
dataset_class1=[1]*125
dataset_class2=[2]*125
dataset_class1=dataset_class1+dataset_class2
dataset_class1=dataset_class1+dataset_class1
#print(dataset)
...
#合并数据集
dataset=np.vstack((dataset,dataset_class1))
print(dataset)
data=dataset.T
data=np.array(data)
print(data)
data=data[:,[7,3,4,1,0,5,6,2,8]]

dataset_not_class=pd.DataFrame(data[:,:-1])
dataset_corr=dataset_not_class.corr()

sns.heatmap(dataset_corr, annot = True, vmin = 0, vmax = 1, cmap = "hot_r",
...
for t in range(0, T):
    ...
    print('\n')
print("---------------------")
sns.heatmap(abs(pd.DataFrame(data[:, :-1][:, [1,3,4,7]]).corr()), annot = True, vmin = 0, vmax = 1, cmap = "hot_r",
            annot_kws = {'size': 8, 'weight': 'bold'})
plt.show()
sns.heatmap(abs(pd.DataFrame(data[:, :-1][:, [0,2,5,6]]).corr()), annot = True, vmin = 0, vmax = 1, cmap = "hot_r",
            annot_kws = {'size': 8, 'weight': 'bold'})
plt.show()

sns.heatmap(abs(pd.DataFrame(data[:, :-1][:, [0,2,5,6,1]]).corr()), annot = True, vmin = 0, vmax = 1, cmap = "hot_r",
            annot_kws = {'size': 8, 'weight': 'bold'})
plt.show()

sns.heatmap(abs(pd.DataFrame(data[:, :-1][:, [0,2,5,6,3]]).corr()), annot = True, vmin = 0, vmax = 1, cmap = "hot_r",
            annot_kws = {'size': 8, 'weight': 'bold'})
plt.show()

sns.heatmap(abs(pd.DataFrame(data[:, :-1][:, [0,2,5,6,4]]).corr()), annot = True, vmin = 0, vmax = 1, cmap = "hot_r",
            annot_kws = {'size': 8, 'weight': 'bold'})
plt.show()

sns.heatmap(abs(pd.DataFrame(data[:, :-1][:, [0,2,5,6,7]]).corr()), annot = True, vmin = 0, vmax = 1, cmap = "hot_r",
            annot_kws = {'size': 8, 'weight': 'bold'})
plt.show()
'''
for i in range(0, len(Group)):
    sns.heatmap(abs(pd.DataFrame(data[:,:-1][:,]).corr()), annot = True, vmin = 0, vmax = 1, cmap = "hot_r",
                annot_kws = {'size': 8, 'weight': 'bold'})
    plt.show()
    print(Group[i])
'''
