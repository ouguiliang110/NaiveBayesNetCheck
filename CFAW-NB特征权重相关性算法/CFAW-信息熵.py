...
class NBayes(object):
    ...
    def getShannonEnt(self, dataSet):
        ...
        for key in labelCounts:
            ...
            self.MeSha.append(prob * math.log(prob, 2))
        print(self.MeSha)
        return shannonEnt
        ...
        for j in dx2:
              for z in dy:
                    if nums_aa[(i,j)]==0:
                       ...
                       I_number+=(nums_aac[(i,j,z)]/lenght)*math.log((nums_aac[(i,j,z)]/lenght)/((nums_aa[(i,j)]/lenght)*(nums_c[(z)]/lenght)))
        return I_number
    ...
    def train(self, vector_data, label_data):
        ...
        #信息熵计算
        #self.getShannonEnt(vector_data)
        for i in range(vector_data.shape[1]):
            self.MeSha.append(self.calc_ent(vector_data[:,i]))
        print(self.MeSha)
        ...
        for i in range(vector_data.shape[1]):
            for j in range(vector_data.shape[1]):
               if  i!=j:
                all_relation1+=self.CountAttributeXY(vector_data[:,i],vector_data[:,j])
    ...
    def predict(self, input_v):
        ...
        for y, p_y in self.p_prior.items():
            ...
            for d, v in enumerate(input_v):  # 0 2 1 0 0 2 1 0
                # print(d, v)
                p *= self.p_condition[(d, v, y)]**(self.Wi[d]*self.MeSha[d])
            p_predict[y] = p
        #     对字典按value进行排序
...
if __name__ == "__main__":
    ...
    X = np.loadtxt('../数据集/[027]wine(0-1).txt')
