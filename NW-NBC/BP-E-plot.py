...
class BPNeuralNetwork:
    ...
    def train(self, cases, labels, theLabel, rmda,limit = 500, learn = 0.05, correct = 0.1,):
        ...
        for j in range(limit):
            ...
            if E > (rmda*hidden_correlation + (1-rmda)*error):
                E = rmda*hidden_correlation + (1-rmda)*error
    ...
    def test(self):
        ...
        my_matrix = np.loadtxt("../数据集/[007]credit(0-1).txt")
        # 将数据集进行归一化处理
        scaler = MinMaxScaler()
        ...
        for rmda in range(1,10,1):
            ...
            sns.heatmap(abs(pd.DataFrame(New_data[:, :-1])).corr(),annot = True)
            plt.show()
            ...
            sns.heatmap(abs(pd.DataFrame(New_data2[:, :-1])).corr(),annot = True)
            plt.show()
