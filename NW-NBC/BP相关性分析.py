...
class BPNeuralNetwork:
    ...
    def test(self):
        ...
        #X.append(p)
        self.setup(m, 10, m)
        Min_matrix = self.train(cases, labels, label_data, 5, 0.05, 0.1)
        ...
        print(pd.DataFrame(New_data[:,:-1]).corr())
        ...
        sns.heatmap(abs(pd.DataFrame(New_data[:,0:8]).corr()),annot = True,vmin = 0,vmax = 1,cmap = "hot_r")
        ...
        my_matrix_normorlize = scaler.transform(New_data2)
        ...
        sns.heatmap(abs(pd.DataFrame(my_matrix_normorlize[:, 0:8]).corr()),annot = True,vmin = 0,vmax = 1,cmap = "hot_r")
        sns.pairplot(pd.DataFrame(my_matrix_normorlize[:, :-1]))
        plt.show()
        ...
        sns.heatmap(abs(pd.DataFrame(vector_data[:,0:8]).corr()),annot = True,vmin = 0,vmax = 1,cmap = "hot_r")
        plt.show()
