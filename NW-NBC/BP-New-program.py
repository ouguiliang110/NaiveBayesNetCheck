...
class BPNeuralNetwork:
    ...
    def back_propagate(self, case, label, learn, correct):
        ...
        for o in range(self.output_n):
            ...
            #print(error)
            output_deltas[o] = sigmoid_derivative(self.output_cells[o]) * (error+self.relation/5000+self.Shannon/2000)
        ...
        for h in range(self.hidden_n):
            ...
            for o in range(self.output_n):
                ...
                #print(error)
            hidden_deltas[h] = sigmoid_derivative(self.hidden_cells[h]) * (error+self.relation/5000+self.Shannon/2000)
    ...
    def train(self, cases, labels, theLabel, rmda, limit = 500, learn = 0.05, correct = 0.1, ):
        E = 10000
        acc = 0
        Min_matrix = []
        for j in range(limit):
            ...
            self.relation=hidden_correlation
    ...
    def test(self):
        ...
        X = np.loadtxt("../数据集/[002]blood(0-1).txt")
        # 将数据集进行归一化处理
        '''
        scaler = MinMaxScaler()
        ...
        for rmda in range(1, 10, 1):
            self.setup(m, 3 * m, m)
            ret = self.train(cases, labels, label_data, rmda / 10, 100, 0.05, 0.1)
