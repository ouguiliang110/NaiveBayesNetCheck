...
class BPNeuralNetwork:
    ...
    def test(self):
        ...
        '''
         归一化
         my_matrix = np.loadtxt("../数据集/[026]vowel(0-1).txt", delimiter = ",", skiprows = 0)
        # 将数据集进行归一化处理
        scaler = MinMaxScaler()
        ...
        for p in range(2,3):
            ...
            LargestAcc = 0
            New_data2 = []
            transformer = LinearDiscriminantAnalysis(n_components = K-1)
            transformer.fit(Min_matrix, label_data)
            X_transformed = transformer.fit_transform(Min_matrix, label_data)
            New_data1 = []
            for i in range(0, n):
