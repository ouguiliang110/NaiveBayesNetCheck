...
class BPNeuralNetwork:
    ...
    def getAccurate(self, HiddenMatrix):
        ...
        return arr_mean,arr_std,arr_mean1,arr_std1,arr_test

    def test(self):
        # 将csv文件导入矩阵当中
        X = np.loadtxt("../NW-NBC/数据集/[023]sonar(0-1).txt")
        ...
        for k in range(0,Num_Floot):
            ...
            rs = random.sample(range(0, m), 8)
            #sns.pairplot(pd.DataFrame(Medium_matrix[:,rs]))
            #plt.show()

            print(m,n)
            for p in range(m+1, 2*m):
                self.setup(m, p, m)
                Min_matrix = self.train(Medium_matrix, Medium_matrix, label_data, 150, 0.05, 0.1)
               ...
               #print('neural-NBC ', correct / n)
                ...
                # ax1=plt.axes()
                # sns.heatmap(pd.DataFrame(New_data[:, :-1]).corr(),ax = ax1)w'w'd
                ...
                for i in range(p, 1, -1):
                    ...
                    New_data1 = []
                    for j in range(0, n):
                        New_data1.append(np.append(X_transformed[j], label_data[j]))
