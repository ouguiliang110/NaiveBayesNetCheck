...
if __name__ == '__main__':
    ...
    test_sample_con=vector_data
    label_data_con=label_data
    # 贝叶斯算法离散化后
    ...
    test_sample_dis = vector_data_NBC
    label_data_dis = label_data
    ...
    for k in range(1):
        ...
        test_data_continuous=vector_data[val_index]
        ...
        for k in range(int(len(IndependentMatrix) * 0.6)):
            ...
            for x in test_sample_dis:
                ...
                if p_predict_sorted[0][0] == label_data_con[temp]:
                    correct += 1
                    correct_matrix_1[temp]=p_predict_sorted
                    ...
                    error_matrix_1[temp]=p_predict_sorted
                temp += 1
            weight=list(correct_matrix_1.keys() & error_matrix_1.keys())
            print(correct_matrix_1.keys() & error_matrix_1.keys())
            print('--------------------------')
            for i in weight:
                print('正确样本'+str(i),correct_matrix_1[i])
                print('错误样本'+str(i),error_matrix_1[i])

            '''
            test_accuracy.append(correct / len(label_data_con))

            correct_matrix_1=np.array(correct_matrix_1)
            correct_matrix_2 = np.array(correct_matrix_2)
            error_matrix_1=np.array(error_matrix_1)
            error_matrix_2 = np.array(error_matrix_2)
            print(correct_matrix_1)
            print(correct_matrix_2)
            print(error_matrix_1)
            print(error_matrix_2)
            ax = plt.subplot(111, projection = '3d')

            ax.scatter(correct_matrix_1[:,0], correct_matrix_1[:,1], correct_matrix_1[:,2], marker='o',c='white',edgecolors='k')  # 绘制数据点
            ax.scatter(correct_matrix_2[:, 0], correct_matrix_2[:, 1], correct_matrix_2[:, 2],marker = ',',c='white',edgecolors='k')
            ax.scatter(error_matrix_1[:,0], error_matrix_1[:,1], error_matrix_1[:,2], marker='o',c='white',edgecolors='r')
            ax.scatter(error_matrix_2[:, 0], error_matrix_2[:, 1], error_matrix_2[:, 2], marker=',',c='white',edgecolors='r')
            plt.show()
        if test_accuracy[-1] > test_accuracy[0]:
            Good_test_Accuracy.append(test_accuracy)
        print(Good_test_Accuracy)
            '''
