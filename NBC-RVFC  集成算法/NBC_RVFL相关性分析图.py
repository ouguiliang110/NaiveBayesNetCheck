...
if __name__ == '__main__':
    ...
    X = np.loadtxt('../数据集/[020]page_small(0-1).txt')

    # X[:, 2] = pd.cut(X[:, 2], Ka, labels = range(Ka))

    Attribute = [1, 2, 3, 4, 7, 8, 9]
    ...
    X = pd.DataFrame(X)
    print(X)
    dum = pd.get_dummies(X[2], prefix = 'key')
    Attri = [0, 1, 3, 4, 5, 6]
    new_df = dum.join(X.iloc[:, Attri])
    print(new_df)
    X = np.array(new_df)

    Xp = pd.DataFrame(X)
    print(Xp)
    Xp.rename(columns = {0: 'A1', 1: 'A2', 2: 'A3', 3: 'A4', 4: 'A5', 5: 'A6', 6: 'A7', 7: 'A8', 8: 'A9', 9: 'A10'},
              inplace = True)
    print(Xp)
    ...
    h = sns.heatmap(abs(Xp.corr()), annot = True, vmin = 0, vmax = 1, cmap = "hot_r",
    ...
    for k in range(0, 1):
        ...
        for i in IndependentMatrix:
          pick.append('A'+str(i+1))
        pick=tuple(pick)
        sns.heatmap(abs(pd.DataFrame(NBTrainingMatrix).corr()),
                    xticklabels = pick,
                    yticklabels = pick, annot = True, vmin = 0, vmax = 1,
                    cmap = "hot_r")
