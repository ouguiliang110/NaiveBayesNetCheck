...
if __name__ == '__main__':
    ...
    for k in range(0, 10):
        ...
        RVFLProbability=rvfl1.predict(RVFLTrainingMatrix, output_prob=True)
        #print(RVFLProbability)
        clf=GaussianNB()
        clf.fit(NBTrainingMatrix,train[1]-1)
        NBProbability=clf.predict_proba(NBTrainingMatrix)
        #print(NBProbability)
        ...
        for i in a:
            Lamuda=i
            ADD_Probability = Lamuda *RVFLProbability + (1-Lamuda)*NBProbability
            # print(ADD_Probability)
            result3 = np.argmax(ADD_Probability, axis = 1)
            # print(result)
            acc3 = np.sum(np.equal(result3, train[1]-1)) / len(train[1])
            arr_test_NBC_RVFL.append(acc3)
            acc.append(acc3)
        plt.plot(a,acc,color='green',label='NBC_RVFL')
        plt.plot(a,acc1,color='red',label='RVFL')
        plt.plot(a,acc2,color='skyblue',label='NBC')
