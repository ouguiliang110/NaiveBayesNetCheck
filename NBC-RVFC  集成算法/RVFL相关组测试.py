...
if __name__ == '__main__':
    ...
    for k in range(0, 10):
        ...
        for i in RelevantMatrix:
            train[0][:, i - 1] = train[0][:, i - 1] * a[p]
            test[0][:, i - 1] = test[0][:, i - 1] * a[p]
            p = p + 1
        ...
        #rvfl和nb集成准确率
        rvfl1.train(train[0], B - 1, num_class)
        RVFLProbability=rvfl1.predict(test[0],output_prob=True)
        result3 = np.argmax(RVFLProbability, axis = 1)
        acc2 = np.sum(np.equal(result3, test[1] - 1)) / len(test[1])
        arr_test_NBC.append(acc2)
