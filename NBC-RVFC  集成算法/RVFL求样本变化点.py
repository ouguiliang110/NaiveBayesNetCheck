...
if __name__ == '__main__':
    ...
    for i in range(int(len(RelevantMatrix)*0.6)):
        ...
        weight = list(correct_matrix.keys() & error_matrix.keys())
        print(correct_matrix.keys() & error_matrix.keys())
        print('--------------------------')
        for i in weight:
            print('正确样本' + str(i), correct_matrix[i])
            print('错误样本' + str(i), error_matrix[i])
