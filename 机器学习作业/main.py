...
def draw_cd_diagram(df_perf=None, alpha=0.05, title=None, cd_name=None, labels=False):
    """
    ...
    cd = compute_CD(average_ranks, 36)
    ...
    graph_ranks_plus(
        ...
        average_ranks.keys(),
        cd=cd,
        cdmethod=None,  # ! 原值4，控制了cd的围绕方法，如果方法被放在csv表的末尾，该值为0.
        reverse=True,
        width=9,
        textspace=1.5,
        labels=labels,
    ...
    plt.savefig(cd_name + '.png', bbox_inches='tight')
...
df_perf = pd.read_csv('统计学秒数2.csv', index_col=False)

draw_cd_diagram(df_perf=df_perf, cd_name=string, labels=True)
